...
function renderNodeSettings(section, data, features, main_node) {
	...
	/* Not core_only (raw core config) - the per-node quick-apply button
	 * below sets uci "main_node" to this node's section id, which is
	 * only meaningful in the normal generate_client.uc flow. In
	 * core_only mode it would silently switch the router OFF the
	 * user's raw JSON config and onto an auto-generated one instead -
	 * a surprising, mode-changing side effect a node-list "Apply"
