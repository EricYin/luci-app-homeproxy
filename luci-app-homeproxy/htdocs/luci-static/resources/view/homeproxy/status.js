			...
			E('button', {
				...
				'click': ui.createHandlerFn(this, () => {
					return L.resolveDefault(callResUpdate(type), {}).then((updRes) => {
						...
						/* Update the version text in place instead of
						 * calling o.map.reset(): a full map reset re-runs
						 * render() for every option in this form, including
						 * the runtime log viewers further down the page,
						 * which call poll.add() on every render with no
						 * matching poll.remove() - each "Check update"
						 * click used to leave one more orphaned polling
						 * interval running in the background forever. */
 ...
 * independent optional cores), so this renders as a handful of
 * "cbi-value-field" rows — one per option, label on the left, just like
 * every other row on this page (Connection check / Resources management) —
 * instead of one big custom block. All rows are built once up front (they
 * share state: busy-lock, update channel, restore button) and then handed
 * out to separate DummyValue options below. */
...
return view.extend({
	render() {
		...
		/* buildCoreContext() reads uci.get('homeproxy','config','core_channel')
		 * to decide which <option> starts selected. That read is only valid
		 * AFTER m.load() has populated the uci cache — which happens inside
		 * m.render(), i.e. AFTER this render() function body has already
		 * run. Calling buildCoreContext() eagerly here (like a first pass
		 * of this refactor did) reads uci too early, always gets back
		 * undefined, and the channel select silently resets to "Latest"
		 * every time regardless of what's actually saved. So instead of
		 * building it now, build it lazily the first time any of these
		 * rows' cfgvalue() actually runs — which the CBI framework only
		 * calls once uci data is loaded — and cache it so the other rows
		 * reuse the same DOM/state instead of rebuilding it five times. */
		let coreCtx = null;
		const ensureCoreCtx = () => {
			if (!coreCtx)
		...
		/* Bind each option's own `o` into the closure by passing it as a
		 * parameter — reusing the loop-scoped `let o` directly here would
		 * mean every one of these cfgvalue functions closes over the SAME
		 * variable, which has already been reassigned to the last option
		 * by the time the framework actually calls them. */
