...
let main_node, main_udp_node, dedicated_udp_node,
    ...
    dns_optimistic, dns_timeout, dns_client_subnet, cache_file_store_dns,
...
/* Dashboard is a normal-mode-only feature: this whole script never runs
 * under main_node="core_only" in the first place (init.d branches to the
 * raw core config path instead), so there's no need to guard this on
 * main_node here too. Always on - no enable/disable switch. Still
 * require the bundled/downloaded web app to actually be present -
 * otherwise this would generate a services entry that just 404s
...
config.dns = {
	...
	/* sing-box >= 1.14: optimistic cache returns a stale answer immediately
	 * while refreshing in the background; mutually exclusive with disable_cache. */
	optimistic: !strToBool(dns_disable_cache) ? strToBool(dns_optimistic) : null,
...
if (!isEmpty(main_node)) {
	...
	if (routing_mode === 'bypass_mainland_china') {
		...
		/* sing-box >= 1.14: a bare "geoip-cn" rule_set on a DNS rule is a
		 * Legacy Address Filter Field and gets rejected at startup once
		 * legacy DNS mode is off. Migrated per the official guide: first
		 * `evaluate` the response for domains not already known to be
		 * non-China, then match the resolved IP against geoip-cn via
		 * match_response instead of matching the domain directly.
		 * Note: once match_response is used anywhere, the deprecated
		 * per-rule "strategy" action field can no longer be mixed into
		 * the same DNS config, so china-dns's own resolution preference
		 * is set via domain_resolver.strategy on the server above instead. */
		push(config.dns.rules, {
...
/* HTTP clients start */
/* sing-box >= 1.14: named HTTP client used by remote rule-set downloads
 * (currently just the China rule-sets below). Replaces the deprecated
 * per-rule-set "download_detour" field. */
if (!isEmpty(main_node))
	config.http_clients = [
		{
			tag: 'main-out',
...
config.route = {
	...
	/* sing-box >= 1.14: replaces the legacy implicit "fall back to the
	 * default outbound" behavior for remote rule-set downloads, which
	 * is deprecated in 1.14.0 and will be removed in 1.16.0. */
...
if (routing_mode === 'bypass_mainland_china') {
	config.experimental = {
		cache_file: {
			enabled: true,
			/* Persistent path (survives reboot), matching init.d's
			 * CACHE_DIR - previously this lived under RUN_DIR (tmpfs),
			 * so the DNS/rule-set cache was silently wiped on every
			 * restart, defeating the point of store_dns. */
			path: HP_DIR + '/cache/cache.db',
			/* sing-box >= 1.14: persists the optimistic DNS cache across restarts */
	...
	config.services = [
		{
			type: 'api',
			tag: 'api-dashboard',
			/* Listen on all interfaces (::) so the LAN-side browser can
			 * reach it at the router's own address, same as every other
			 * HomeProxy-exposed port (mixed_port, tproxy_port, etc). */
			listen: '::',
