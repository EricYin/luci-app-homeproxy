...
/* Streams the response through curl instead of buffering it in a temp file,
 * enforces a hard size cap so a malicious/huge response can't exhaust memory,
 * and optionally routes the request through one of our own outbound nodes
 * (proxyUrl, e.g. "socks5h://127.0.0.1:1080") so subscription updates can
 * succeed even when the direct route to the sub host is blocked. */
