...
/* Mihomo/Clash YAML subscription support start (ported from upstream,
 * parsing only - the auto-cleanup/reconciliation logic that came with it
 * was intentionally left out). parse_mihomo_yaml() extracts each
 * flow-style "- {...}" proxy entry under a "proxies:" key without a full
 * YAML parser, then parse_mihomo_proxy() translates Clash/Mihomo field
 * names into this fork's node config, mirroring parse_uri() below. */
...
/* Convert a single-line YAML flow-style mapping/sequence (e.g.
 * "{name: HK 01, type: ss, server: 1.2.3.4, port: 443, udp: true}") into
 * valid JSON so it can be handed to json(). Real-world Clash/Mihomo
 * subscriptions almost always use bare (unquoted) keys and bare string
 * values, which is NOT legal JSON - a plain json() call on that text
 * throws and silently drops the node. This walks the fragment
 * character-by-character, quoting bare keys/values as needed, while
 * leaving already-quoted strings, numbers, booleans and null alone.
 * Nested {} / [] are supported; multi-line block-style YAML is not (that
...
function yaml_flow_to_json(s) {
	...
	let out = '';
	let i = 0;
	let stack = [];         /* nesting stack: '{' or '[' */
	let expect_key = false; /* true right after '{' or ',' inside an object */
	...
	while (i < len) {
		...
		/* bare token (unquoted key or value): read until the next
...
function parse_mihomo_yaml(text) {
	...
	for (let line in split(text, '\n')) {
		...
		/* stop once we leave the proxies list, i.e. hit the next
		 * top-level key. Allow hyphens so keys like "proxy-groups:"
		 * and "rule-providers:" are recognized too. */
...
/* Mihomo/Clash YAML subscription support end */
...
function main() {
	...
	for (let url in subscription_urls) {
		...
		if (mihomo_nodes) {
			/* Clash/Mihomo YAML subscription (flow-style "proxies:" list) */
			nodes = mihomo_nodes;
		} else {
			try {
	...
	/* "core_only" isn't an actual node uci-section name, so the "is my
	 * selected node still around" reconciliation below doesn't apply to
	 * it - uci.get(uciconfig, main_node) would always come back empty,
	 * incorrectly bouncing main_node over to the first available node
